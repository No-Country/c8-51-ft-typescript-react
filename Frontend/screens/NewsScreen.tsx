import { View, StyleSheet, TouchableOpacity, ScrollViewComponent, ScrollView } from "react-native";
import { StatusBar } from "expo-status-bar";
import { useTheme, Button, withTheme, Card, Title, Paragraph, IconButton, Appbar, Searchbar } from "react-native-paper";
...
export default function NewsSceeen() {
  ...
  const onChangeSearch = (query) => {
    ...
    if (query.length > 0) {
      setNewsState(NewsJson.filter((news) => news.tittle.toLowerCase().includes(query.toLowerCase()) || news.paragraph.toLowerCase().includes(query.toLowerCase())));
    }
  };
  const onToggleFav = () => {
    setOnlyFav(!onlyFav);
  ...
  return (
    <>
      <SafeAreaView
        style={{ backgroundColor: theme.colors.light, flex: 1 }}
      >
        <View style={{
          flexDirection: "row",
